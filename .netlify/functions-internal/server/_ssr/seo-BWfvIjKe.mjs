import { n as getPost, t as POSTS } from "./posts-D9Z9S4v5.mjs";
import { n as SERVICES } from "./services-CC8WogQK.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/seo-BWfvIjKe.js
/**
...
*/
var SITE_URL = "https://pocketstudio.biz";
...
var HAIR_SALON_JSONLD = {
	...
	name: BUSINESS.name,
	description: "Solo house-call hair artist in Seattle, WA. Cuts and color, all priced at the chair. $25 booking deposit holds your slot.",
	url: `${SITE_URL}/`,
...
var BLOG_JSONLD = {
	...
	description: "Dated notes from one chair and one brain: house-call updates, the hunt for a new chair, and color stories.",
	url: `${SITE_URL}/blog`,
	author: {
		"@type": "Person",
		name: BUSINESS.founderName,
...
function blogPostingJsonLd(slug) {
...
function buildHead(path, seo) {
	...
	if (seo.jsonLd && seo.jsonLd.length > 0) head.scripts = seo.jsonLd.map(jsonLd);
	return head;
}
var STATIC_ROUTES = {
	"/": {
	...
	"/book": {
		...
		description: "Pick a service, a day, and a time — a $25 deposit holds your slot and comes off your total. Real calendar, instant confirmation, house calls across Seattle."
	},
	"/studio": {
	...
	"/classic": {
		...
		description: "The original Pocket Studio booking page, preserved. Book cuts + color directly with MyKey, a solo house-call hair artist in Seattle, WA."
	},
	"/mobile": {
		title: "Mobile Booking — Cuts & Color in Seattle · Pocket Studio",
		description: "Book cuts and color with MyKey Pocket in Seattle. House calls only. Tap a service, pick a slot, done."
	},
	"/blog": {
		title: "Updates — Dispatches from the Chair · Pocket Studio",
		description: "Dated notes from one chair and one brain: house-call updates, the hunt for a new chair, and color stories. Signed, always, — mykey.",
		jsonLd: [BLOG_JSONLD]
	},
	"/privacy": {
		title: "Privacy Policy · Pocket Studio",
		description: "How Pocket Studio collects, uses, and protects your information when you book cuts + color with MyKey in Seattle."
	},
	"/terms": {
		title: "Terms of Service · Pocket Studio",
		description: "The fine print: 24-hour cancellation, no-show policy, deposits, house-call terms, and booking rules for Pocket Studio, Seattle."
	}
};
var DEFAULT_SEO = {
...
function headFor(route) {
	...
	if (path.startsWith("/services/")) {
		...
		if (service) return buildHead(path, {
			title: `${service.name} in Seattle — House Calls · ${SITE_NAME}`,
			description: `${service.blurb} ${service.name} — ${service.duration}, ${service.price}. Book direct with MyKey, house-call hair artist in Seattle, WA.`
		});
		return buildHead(path, {
			title: `Service Not Found · ${SITE_NAME}`,
			description: "That service isn't on the menu. Cuts and color house calls across Seattle — the full menu lives on the booking page.",
			noindex: true
		});
	}
	if (path.startsWith("/blog/")) {
		...
		if (post) {
			...
			return buildHead(path, {
				...
				description: post.excerpt,
				type: "article",
				jsonLd: posting ? [posting] : void 0
			});
		}
		return buildHead(path, {
			title: `Dispatch Not Found · ${SITE_NAME}`,
			description: "That dispatch doesn't exist — all updates live on the updates page.",
			noindex: true
		});
	}
