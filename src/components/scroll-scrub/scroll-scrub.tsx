/* Scroll scrub — seam-locked scroll movie controller.
 * Adapted from the A4 scroll-scrub reference (higgsfield-websites skill).
 * Poster mode: scenes without a `clip` run a slow Ken Burns drift on the
 * poster instead of video seeks, so the page ships today and generated
 * clips drop in later by filling clip/mobileClip paths. */
...
export interface ScrollScrubScene {
  id: string;
  label: string;
  /** Exact first frame of the deployed desktop clip (or placeholder art). */
  poster: string;
...
function buildSegments(scenes: ScrollScrubScene[]): Segment[] {
...
export function ScrollScrub({ scenes, theme, className, onActiveSectionChange }: ScrollScrubProps) {
  ...
  useEffect(() => {
    ...
    const runtime: RuntimeSegment[] = segments.map((segment, index) => ({
      ...
      loading: false,
      posterEl:
        layerNodes[index].querySelector<HTMLImageElement>(".scroll-scrub__poster") ?? undefined,
      ready: false,
      start: 0,
      target: 0,
      visible: index === 0,
    ...
    const readScroll = () => {
      ...
      if (current.sectionIndex !== active) {
        active = current.sectionIndex;
    ...
    const updateMedia = () => {
      for (const segment of runtime) {
        ...
        // Poster mode: slow Ken Burns push-in driven by the same progress.
        if (!reduceMotion && segment.posterEl) {
          const p = segment.current;
          const scale = 1.05 + 0.09 * p;
          const tx = segment.drift * (p - 0.5) * 2.4;
          const ty = (p - 0.5) * -1.6;
          segment.posterEl.style.transform = `scale(${scale.toFixed(4)}) translate(${tx.toFixed(3)}%, ${ty.toFixed(3)}%)`;
        }
      }
    };
    ...
    return () => {
      ...
      for (const segment of runtime) {
        ...
        if (segment.posterEl) {
          segment.posterEl.style.removeProperty("transform");
        }
      }
    };
  }, [segments]);
