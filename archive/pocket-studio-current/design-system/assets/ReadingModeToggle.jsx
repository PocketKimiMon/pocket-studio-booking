...
const RMT_KEY = 'ps-reading-mode', RMT_VALUE = 'dyslexic', RMT_OFF = 'off', RMT_ATTR = 'data-reading-mode';
// DEFAULT ON: no stored value means dyslexic mode is active.
const rmtRead = () => { try { return localStorage.getItem(RMT_KEY) !== RMT_OFF; } catch (e) { return true; } };
const rmtPersist = (on) => { try { localStorage.setItem(RMT_KEY, on ? RMT_VALUE : RMT_OFF); } catch (e) {} };
const rmtApply = (on) => {
  const r = document.documentElement;
  on ? r.setAttribute(RMT_ATTR, RMT_VALUE) : r.removeAttribute(RMT_ATTR);
};

export function ReadingModeToggle({ compact = false, className = '' }) {
  const [on, setOn] = React.useState(rmtRead());
  React.useEffect(() => { rmtApply(on); }, []); // sync on mount
  const set = (next) => {
    setOn(next); rmtApply(next); rmtPersist(next);
